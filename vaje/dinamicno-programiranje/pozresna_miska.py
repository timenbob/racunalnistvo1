...
#     13
# =============================================================================
def pozresna_miska(tabela):
    # for el in tabela:
    #     el.extend([-1])
    # tabela.append([-1]*len(tabela[0]))
    ...
    def pomozna(i,j):
        ...
        if i>n or j>m:
            return -float("inf")
        if (i,j) not in cache:
            cache[(i,j)]=max(pomozna(i+1,j),pomozna(i,j+1))+tabela[i][j]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODUwNCwidXNlciI6NTM2MX0:1rSjWt:d4TDZ9fE8anSSzLQb7fnEQXQ18sxZp20mVYOMzo8yXU"
        try:
            Check.equal('pozresna_miska([[1,2,0], [2,4,5], [7,0,1]])', 13)
            Check.equal('pozresna_miska([[1]])', 1)
            Check.equal('pozresna_miska([[1, 2], [1, 2]])', 5)
            
            Check.equal('pozresna_miska([[(i+j) % 42 for i in range(123)] for j in range(123)])', 4900)
        except TimeoutError:
