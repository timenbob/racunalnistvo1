...
def zadnji_vozel(prvi):
    sez=vrni_seznam(prvi)
    if sez==[]:
        return None
    else:
        el=sez[-1]
        return iz_seznama([el])
# =====================================================================@038368=
# 3. podnaloga
...
#     ['četrtek', 'petek']
# =============================================================================
def pogojni_seznam(prvi, pogoj):
    sez=[]
    while prvi != None:
        if pogoj(prvi.podatek):
            sez.append(prvi.podatek)
        prvi=prvi.naslednji
    return sez
# =====================================================================@038370=
...
def je_urejen(prvi):
    if prvi==None:
        ...
        prejsni=prvi.podatek
        
        while prvi != None:
            if prejsni>prvi.podatek:
                return False
            prejsni=prvi.podatek
            prvi=prvi.naslednji
        return True
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM2NiwidXNlciI6NTM2MX0:1r124a:WETJpvlXQAkfSPZjhSo7fUHm05fEqsulW-hBZQ7-UqU"
        try:
            test_data = [
                (["n = dolzina(None)"],
                 {'n': 0}),
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM2NywidXNlciI6NTM2MX0:1r124a:17wEfNCfOyjbVlVNaTV5PTyBCRI7kfbngSP0x3jfzg4"
        try:
            test_data = [
                (["v = iz_seznama(['sreda', 'četrtek', 'petek', 'sobota'])",
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM2OCwidXNlciI6NTM2MX0:1r124a:b4Fy5SmJTeJjNbsJhyzfkF-W-6m-T_D6vxxPbhw1E78"
        try:
            test_data = [
                ('seznam_sodih(None)', []),
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM2OSwidXNlciI6NTM2MX0:1r124a:5CEHIsm20xjq8iLc-fLSmMb25NvdjSYGcJXPuh6PZUo"
        try:
            test_data = [
                (["v = iz_seznama(['sreda', 'četrtek', 'petek', 'sobota'])",
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM3MCwidXNlciI6NTM2MX0:1r124a:5t1RpE70ZBRBvjuSVSJAbduPnxZvjfUuIcf7C6MjTZ0"
        try:
            test_data = [
