...
def sodi_in_lihi(začetek):
    ...
    while kam is not None:
        if kam.podatek is not None:
            if kam.podatek%2==0:
                if sodi is None:
                    sodi=Vozel(kam.podatek)
                    kam_sodi=sodi
                else:
                    kam_sodi.naslednji = Vozel(kam.podatek)
                    kam_sodi = kam_sodi.naslednji
            else:
                if lihi is None:
                    lihi=Vozel(kam.podatek)
                    kam_lihi=lihi
                else:
                    kam_lihi.naslednji = Vozel(kam.podatek)
                    kam_lihi = kam_lihi.naslednji
        kam = kam.naslednji
...
def odpni_zadrgo(v):
    ...
    for el in vrni_seznam(v):
        if bol:
            ...
            sez2.append(el)
            bol=True    
    return(iz_seznama(sez1),iz_seznama(sez2))
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM3MSwidXNlciI6NTM2MX0:1r124a:z0XeU7f9SdrACJZjVV7lExmZ4RiBkPw_VyGnlXrj3Mk"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM3MiwidXNlciI6NTM2MX0:1r124a:vRSB8fkysrjp8r_UqTrUPGjf6ERbEUtipEJ5E0BAAJ8"
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2, 1, 3, 4, 9, 8])",
                  "v1, v2 = sodi_in_lihi(v)",
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM3MywidXNlciI6NTM2MX0:1r124a:ZBmAgUQzLX2ibLuISfRu2HriEe8ibSeTiomOEsjM19M"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM3NCwidXNlciI6NTM2MX0:1r124a:KHUI8XaleunHy_I4_En7-BCL4d9a-l7RG__fwDnEPgs"
        try:
            test_data = [
                (["v1 = iz_seznama([1, 2, 3])",
                  "v2 = iz_seznama([4, 5, 6])",
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM3NSwidXNlciI6NTM2MX0:1r124a:RjG9HjIowqLxMyQinWoVE0S5bNdIEw63rGKrzR4EorU"
        try:
            test_data = [
                (["v1 = iz_seznama([7, 5, 2])",
                  "v2 = iz_seznama([1, 3, 4, 9, 8])",
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM3NiwidXNlciI6NTM2MX0:1r124a:E9gAkLchCS-bme3i5BYgfuM2nSfc0JfKK3vne5O6Iro"
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2, 1, 3, 4, 9, 8])",
                  "v1, v2 = odpni_zadrgo(v)",
