...
def drevo_vmesni_obratni(vmesni, obratni):
    if len(obratni)==0:
        ...
        koren=obratni[-1]
        i=vmesni.index(koren)
        levo=drevo_vmesni_obratni(vmesni[:i],obratni[0:i])
...
def drevo_vmesni_nivojski(vmesni, nivojski):
    if len(nivojski)==0:
        ...
        nivojskiL=[]
        nivojskiD=[]
        for el in nivojski[1:]:
            if el in vmesni[:i]:
                nivojskiL.append(el)
            else:
                nivojskiD.append(el)
        levo=drevo_vmesni_nivojski(vmesni[:i],nivojskiL)
        desno=drevo_vmesni_nivojski(vmesni[i+1:],nivojskiD )
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ3NiwidXNlciI6NTM2MX0:1rDin8:0H7k0KIIhF89jeM2JWY5Ohtk7xb5MgO6ygzuQpoANYk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ3NywidXNlciI6NTM2MX0:1rDin8:JnwpSuqy_l4wyoZNB3lnhlhQqu13wrnu2vr3PJEGvbc"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ3OCwidXNlciI6NTM2MX0:1rDin8:CZFLaLZtI4sL-DOLVzXuC7vsVSx6KJ4zhoN4TNww2tY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ3OSwidXNlciI6NTM2MX0:1rDin8:UpOT0BkEetgnGTstxV8v4SnoNbBIKc5DF4qVWExu_e0"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ4MCwidXNlciI6NTM2MX0:1rDin8:e_5k2SycXi8F6BE4eMv-1LGjxZk1tOHq42O991doN5Y"
        try:
