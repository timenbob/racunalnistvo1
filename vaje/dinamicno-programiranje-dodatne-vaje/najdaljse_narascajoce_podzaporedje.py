...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODUyOCwidXNlciI6NTM2MX0:1rTk6W:fOON8SIuCFEPRzcg2fALV0A2WITMZ-GII6Xh6c_LilU"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODUyOSwidXNlciI6NTM2MX0:1rTk6W:ut6sx0T_ee20sNiNIImXvkqCHi_RutKYnpThcVtxRWc"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODUzMCwidXNlciI6NTM2MX0:1rTk6W:4nmBBQ4gk9V_iihmEKyaSbbQjAiZOVlSJSA7-gnuoR4"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODUzMSwidXNlciI6NTM2MX0:1rTk6W:26KvwZ_Q5h9h2ayOOMbOhUpSNo1yB0uzmdBuWsOZTNU"
        try:
            tests = [('max_podzaporedje_vsota([1, 101, 2, 3, 100, 4, 5])', 106),
                     ('max_podzaporedje_vsota([3, 4, 5, 10])', 22),
                     ('max_podzaporedje_vsota([10, 5, 4, 3])', 10),
                     ('max_podzaporedje_vsota([])', 0)
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODUzMiwidXNlciI6NTM2MX0:1rTk6W:1h4kgwD7SX59bNDuFpgQP-hf_73ToOtSNC6jMEG2S7U"
        try:
            tests = [('max_podzaporedje_lihi([15, 27, 14, 38, 63, 55, 46, 65, 85])', [15, 27, 63, 65, 85]),
                     ('max_podzaporedje_lihi([50, 3, 10, 7, 40, 80])', [3, 7]),
                     ('max_podzaporedje_lihi([0, 8, 4, 12, 2, 10, 6, 14, 1, 9, 5, 13, 3, 11, 7, 15])', [1, 9, 13, 15]),
                     ('max_podzaporedje_lihi([0, 8, 4, 12, 2, 10, 6, 14])', []),
                     ('max_podzaporedje_lihi([])', [])
                     ]
