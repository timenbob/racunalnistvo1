...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODMwNSwidXNlciI6NTM2MX0:1qtP6T:6jXgajTnWa89c5swlTX7jShW1LKfDvEhihMcJyxF74s"
        try:
            Check.equal("""je_ogrlica('', 0, 0)""", True) and \
            Check.equal("""je_ogrlica('BBBB', 4, 0)""", True) and \
            Check.equal("""je_ogrlica('BBBB', 0, 4)""", False) and \
            Check.equal("""je_ogrlica('BBRBBRB', 5, 2)""", True) and \
            Check.equal("""je_ogrlica('RRBBBBB', 5, 2)""", True) and \
            Check.equal("""je_ogrlica('BBRBBRB', 2, 5)""", False) and \
            Check.equal("""je_ogrlica('BBRBBRBBB', 5, 2)""", False) and \
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODMwNiwidXNlciI6NTM2MX0:1qtP6T:jqUUeCoOM552xV33rsW80zLojWwN7DiW0heHSDfr2r4"
        try:
            Check.equal('stevilo_ogrlic(5, 0)', 1) and \
            Check.equal('stevilo_ogrlic(3, 3)', 20) and \
            Check.equal('stevilo_ogrlic(5, 2)', 21) and \
            Check.equal('stevilo_ogrlic(4, 2)', 15) and \
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODMwNywidXNlciI6NTM2MX0:1qtP6T:j_cM0u5XZYWYI2xLB0rfJkpqCNMMUkt1NG13l6pWpX0"
        try:
            Check.equal('ogrlice(2, 2)', ["BBRR", "BRBR", "BRRB", "RBBR", "RBRB", "RRBB"]) and \
            Check.equal('ogrlice(2, 3)', ['BBRRR', 'BRBRR', 'BRRBR', 'BRRRB', 'RBBRR', 'RBRBR', 'RBRRB', 'RRBBR', 'RRBRB', 'RRRBB']) and \
            Check.equal('ogrlice(4, 4)[:3]', ["BBBBRRRR", "BBBRBRRR", "BBBRRBRR"]) and \
            Check.equal('ogrlice(4, 0)', ["BBBB"]) and \
            Check.equal('ogrlice(0, 4)', ["RRRR"]) and \
            Check.equal('ogrlice(5, 2)[:10]', ['BBBBBRR', 'BBBBRBR', 'BBBBRRB', 'BBBRBBR', 'BBBRBRB', 'BBBRRBB', 'BBRBBBR', 'BBRBBRB', 'BBRBRBB', 'BBRRBBB'])
            if not Check.equal('len(ogrlice(5, 2))', 21):
                Check.feedback('Za ogrlice(5, 2) moramo dobiti 21 kombinacij, mar ne?')
        except:
            Check.error(
                "Testi sprožijo izjemo\n  {0}",
                "\n  ".join(traceback.format_exc().split("\n"))[:-2],
