...
class DruzabnaVrsta:
    ...
    def dodaj_prisleka(self, ime, skupina):
        ...
        ista_skupina=False
        self.vrsta.vstavi(None)
        vstavljen=False
        
        while self.vrsta.zacetek() is not None:
            ime_i, skupina_i=self.vrsta.zacetek()
            self.vrsta.odstrani()
            if skupina_i==skupina:
                ista_skupina=True
            if ista_skupina and  skupina_i != skupina:
                self.vrsta.vstavi((ime, skupina))  
                ista_skupina=False 
                vstavljen=True
            self.vrsta.vstavi((ime_i, skupina_i))  
        self.vrsta.odstrani()
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM0OCwidXNlciI6NTM2MX0:1qyVRP:n1z_gOyIHmub-7O9re-75nmtQbiNUvXvYVA0Ggm5VTo"
        try:
