...
def vsota(drevo):
    if drevo.prazno:
        return 0
    if not drevo.prazno:
        return drevo.podatek+vsota(drevo.desno)+vsota(drevo.levo)
# =====================================================================@038441=
# 2. podnaloga
...
def minimum(drevo):
    if drevo.prazno:
        ...
        return min(drevo.podatek,minimum(drevo.desno))
    elif drevo.desno.prazno:
        return min(drevo.podatek,minimum(drevo.levo))
    else:
        return min(drevo.podatek, minimum(drevo.desno),minimum(drevo.levo))
# =====================================================================@038443=
# 4. podnaloga
...
def premer(drevo):
    if drevo.prazno:
        ...
        return 1+premer(drevo.desno)
    elif drevo.desno.prazno:
        return 1+premer(drevo.levo)
    else:
        return 2+ premer(drevo.levo)+premer(drevo.desno)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ0MCwidXNlciI6NTM2MX0:1rBBSU:Cpi-XxxPvam2kJrwSYQhw_9A_fGhaV5FjZKGC690phE"
        try:
            test_data = [
                ('vsota(Drevo())', 0),
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ0MSwidXNlciI6NTM2MX0:1rBBSU:P92ZDUAfhrgSGx-TyQJXdCx2dosh5YmmO3n2Ah2wSGQ"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ0MiwidXNlciI6NTM2MX0:1rBBSU:CPH3tYTFe0LT7XXIo-0nGMzPJ-dTJWvIOvZloNZDPIs"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ0MywidXNlciI6NTM2MX0:1rBBSU:H_JsHG0mJCt0YAW_QB-zNYKxKaibRT8gBRwfQAua8aY"
        try:
            test_data = [
                ...
                ("premer(Drevo(3, desno=Drevo(2, levo=Drevo(2, levo = Drevo(1, levo = Drevo(9))), desno = Drevo(1, desno=Drevo(1, desno = Drevo(2))))))", 6),
