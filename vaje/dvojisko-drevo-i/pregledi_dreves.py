...
# `drevo` v _premem vrstnem redu_ (pre-order). To pomeni, da najprej obiščemo
...
# `drevo` v _vmesnem vrstnem redu_ (in-order). To pomeni, da najprej obiščemo
...
def vmesni_pregled(drevo):
    if drevo.prazno:
        ...
        return(vmesni_pregled(drevo.levo))+[drevo.podatek]+(vmesni_pregled(drevo.desno))
# =====================================================================@038449=
# 3. podnaloga
# Sestavite generator `obratni_pregled(drevo)`, sestavi tabelo elementov drevesa
# `drevo` v _obratnem vrstnem redu_ (post-order). To pomeni, da najprej
...
def obratni_pregled(drevo):
    if drevo.prazno:
        ...
        return(obratni_pregled(drevo.levo))+(obratni_pregled(drevo.desno))+[drevo.podatek]
# =====================================================================@038450=
# 4. podnaloga
# Sestavite funkcijo `pregled_po_nivojih(drevo)`,  sestavi tabelo elementov drevesa
# `drevo` v zaporedju _po nivojih_ (level-order). To pomeni, da najprej obiščemo koren,
...
def pregled_po_nivojih(drevo):
    if drevo.prazno:
        ...
        return([drevo.podatek]+pregled_po_nivojih(drevo.levo))+(pregled_po_nivojih(drevo.desno))
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ0NywidXNlciI6NTM2MX0:1rBBSU:_Rq9dp42D2IRTaTnUUfKGHNFn3rDX6oE7yNdu3wWtMY"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ0OCwidXNlciI6NTM2MX0:1rBBSU:1I6aQQ764U2A4jh6ORat03gbGbsjp2WA25UmgmLw0l0"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ0OSwidXNlciI6NTM2MX0:1rBBSU:wiEKNL4qCSnlkySpwPP8H8T9aO_f5HIlAsAJ-woI4Nk"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ1MCwidXNlciI6NTM2MX0:1rBBSU:t5f6SsyzPXUw17UujO4PHTED7rmyVs5cmr4GSdYm820"
        try:
            test_data = [
