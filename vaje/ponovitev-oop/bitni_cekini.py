...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODIzMSwidXNlciI6NTM2MX0:1qqrqd:KcrY5ld-fEXTCVxWgoe0U6OpsKq7nmOV8Qf-2rcUyGA"
        try:
            tests = [
                    ('BitniCekin(5)._stanje', 5),
                    ('BitniCekin(1000)._stanje', 1000),
                    ('BitniCekin()._stanje', 0)
                    ]
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODIzMiwidXNlciI6NTM2MX0:1qqrqd:b2b8RPZFXgGTu44bInk7pbrnt6GmiRB9ZMmgHZW8Pr8"
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODIzMywidXNlciI6NTM2MX0:1qqrqd:pUzeAtkT-XZForVW-SwhiOOlWL2R82igJBmNUmu_Q7U"
        try:
            Check.run(["racun = BitniCekin(10)", "prvi_dvig = racun.dvig(5)",
                       "drugi_dvig = racun.dvig(10)", "polog = racun.polog(30)",
                       "tretji_dvig = racun.dvig(20)", "konec = racun._stanje"],
                      {'prvi_dvig': True, 'drugi_dvig': False, 'polog': 35,
                       'tretji_dvig': True, 'konec': 15})
            Check.run(["racun = BitniCekin()", "prvi_dvig = racun.dvig(5)",
                       "drugi_dvig = racun.dvig(10)", "polog = racun.polog(15)",
                       "tretji_dvig = racun.dvig(20)", "konec = racun._stanje"],
                      {'prvi_dvig': False, 'drugi_dvig': False, 'polog': 15,
                       'tretji_dvig': False, 'konec': 15})
            Check.run(["racun = BitniCekin()", "prvi_dvig = racun.dvig(5)",
                       "drugi_dvig = racun.dvig(10)", "polog = racun.polog(0)",
                       "tretji_dvig = racun.dvig(20)", "konec = racun._stanje"],
                      {'prvi_dvig': False, 'drugi_dvig': False, 'polog': 0,
                       'tretji_dvig': False, 'konec': 0})
        except:
            Check.error(
                "Testi sprožijo izjemo\n  {0}",
                "\n  ".join(traceback.format_exc().split("\n"))[:-2],
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODIzNCwidXNlciI6NTM2MX0:1qqrqd:S7yEcf0R-X2MI2WloXfHHCu4rVz_H4NWlGahJXmlL0k"
        try:
            Check.run(["racun1 = BitniCekin(10)", "racun2 = BitniCekin(10)",
                      "uspeh1 = prenesi(racun1, racun2, 7)", "uspeh2 = prenesi(racun2, racun1, 20)",
                       "konec1 = racun1._stanje", "konec2 = racun2._stanje"],
