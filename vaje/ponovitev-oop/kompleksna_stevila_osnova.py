...
class KompleksnoStevilo(KompleksnoStevilo):
    def polarni_zapis(self):
        if (self._im <= 0 and self._re <0 ):
            return(math.sqrt(self._im**2+self._re**2),math.atan(self._im/self._re)+math.pi)
        elif(self._im >= 0 and self._re >0 ):
            return(math.sqrt(self._im**2+self._re**2),math.atan(self._im/self._re))
        elif(self._im >= 0 and self._re <0 ):
            return(math.sqrt(self._im**2+self._re**2),math.atan(self._im/self._re)+math.pi)
        elif (self._im <= 0 and self._re >0 ):
            return(math.sqrt(self._im**2+self._re**2),math.atan(self._im/self._re)+math.pi*2)
        elif(self._re==0):
            if self._im>=0:
               return(math.sqrt(self._im**2+self._re**2),math.pi/2) 
            else:return(math.sqrt(self._im**2+self._re**2),math.pi*6/4)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODI1OSwidXNlciI6NTM2MX0:1qqrqd:COslxLnPah-adbI3C4B233bjTkoeRdOPg7OOo7TTDKQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODI2MCwidXNlciI6NTM2MX0:1qqrqd:yn3edfoQTwWPwFz3eRX2dLiE_00WEjfSTPAuiH9qGec"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODI2MSwidXNlciI6NTM2MX0:1qqrqd:4rdyWe3oXjhW4sk7DX70rtBPymWX4W2hVVWnMGBnFtE"
        try:
