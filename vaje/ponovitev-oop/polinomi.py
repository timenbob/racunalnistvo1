...
# $x$. Polinom predstavimo s seznamom njegovih koeficientov, kjer je $k$-ti
# element seznama koeficient pri $x^k$. Na primer, polinom $x^3 + 2 x + 7$
...
# Stopnja ničelnega polinoma je v matematiki po dogovoru $-\infty$, pri nas pa
# bo $-1$.
# =============================================================================
...
# pokrajšajo: $(x^3 + 2 x + 7) + (- x^3 - 2 x + 10) = 17$
# =============================================================================
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODIzOSwidXNlciI6NTM2MX0:1qqrqd:I0wEJh3gYY1BLiM9pBHVdJkYVZWJ2JqTehVkqCQfQoc"
        try:
            tests = [
                    ('Polinom([1, 2, 3])._koef', [1, 2, 3]),
                    ('Polinom([1, 2, 0, 0])._koef', [1, 2])
                    ]
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODI0MCwidXNlciI6NTM2MX0:1qqrqd:598vyZDUZVunQ2J_QckA8kVBl_cFjVKoMm6W7Eh-yfU"
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODI0MSwidXNlciI6NTM2MX0:1qqrqd:Y4-JPemQXy2Wmrmm6YbW-tHnOYB7M4rVIcqNhJ88MGE"
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODI0MiwidXNlciI6NTM2MX0:1qqrqd:PF0kLjFepMQ41I1VG6f1F9oFgiriFB4hEAbJIpmI1hc"
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODI0MywidXNlciI6NTM2MX0:1qqrqd:g0m0rzmXY3fbLxxcHV8cQsRyZakuW-KfnQoULrv2tJQ"
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODI0NCwidXNlciI6NTM2MX0:1qqrqd:yx4TLu7z-6BsGUpJFpEB5B8OQLGJ_JsH0xIO9AEyv6Q"
        try:
            tests = [
                    ('str(Polinom([1, 2, 3]))', "3 x^2 + 2 x + 1"),
                    ('str(Polinom([1, 3]))', "3 x + 1"),
                    ('str(Polinom([1, -2, 3, -1]))', "-x^3 + 3 x^2 - 2 x + 1"),
                    ('str(Polinom([1, 0, 0, -1]))', "-x^3 + 1"),
                    ('str(Polinom([0, 0, 0, -5]))', "-5 x^3"),
                    ('str(Polinom([-1, 2, -3, 1]))', "x^3 - 3 x^2 + 2 x - 1")
                    ]
